...
const DAILY_TIMEOUT = inMilliseconds(24, Duration.Hour);
...
> {
  ...
  private _snapIds: Map<string, string>;

  private _dailyTimer!: Timer;
  ...
  private schedule(job: Cronjob) {
    ...
    this._snapIds.set(job.id, job.snapId);
  }
  ...
  unregister(snapId: SnapId) {
    const jobs = [...this._snapIds.entries()].filter(
  ...
  async dailyCheckin() {
    ...
    this._dailyTimer = new Timer(DAILY_TIMEOUT);
    this._dailyTimer.start(() => this.dailyCheckin());
  }
